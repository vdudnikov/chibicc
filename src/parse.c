...
static Node *compute_vla_size(Type *ty, Token *tok)
{
    ...
    Node *expr = new_binary(
        ND_ASSIGN, new_var_node(ty->vla_size, tok), new_binary(ND_MUL, ty->vla_len, base_sz, tok),
        tok);
    ...
    if (ty->kind == TY_STRUCT) {
        for (Member *mem = ty->members; mem; mem = mem->next) {
            if (mem->is_bitfield) {
                ...
                cur = write_gvar_data(
                    cur, init->children[mem->idx], mem->ty, buf, offset + mem->offset);
            }
        }
        return cur;
    }
...
static Node *to_assign(Node *binary)
{
    ...
    if (binary->lhs->ty->is_atomic) {
        ...
        cur = cur->next = new_unary(
            ND_EXPR_STMT,
            new_binary(
                ND_ASSIGN, new_var_node(addr, tok), new_unary(ND_ADDR, binary->lhs, tok), tok),
            tok);
        ...
        cur = cur->next = new_unary(
            ND_EXPR_STMT,
            new_binary(
                ND_ASSIGN, new_var_node(old, tok),
    ...
    Node *expr2 = new_binary(
        ...
        new_binary(
            binary->kind, new_unary(ND_DEREF, new_var_node(var, tok), tok), binary->rhs, tok),
        tok);
